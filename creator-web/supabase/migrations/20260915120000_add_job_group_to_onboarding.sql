-- 온보딩 2단계에서 선택 입력으로 받는 직업군을 프로필에 저장한다.
-- 직업군은 선택 항목이므로 미입력은 NULL 로 남기고, 값이 있으면 공백을 제거해 저장한다.
BEGIN;
...
ALTER TABLE public.users
  DROP CONSTRAINT IF EXISTS users_job_group_valid;

ALTER TABLE public.users
  ADD CONSTRAINT users_job_group_valid CHECK (
    job_group IS NULL
