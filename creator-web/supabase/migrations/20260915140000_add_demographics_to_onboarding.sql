-- 온보딩 3단계에서 받는 성별(필수), 연령대(필수), 주 사용기기(선택)를 프로필에 저장한다.
-- 기존 회원은 값이 없으므로 컬럼은 NULL 을 허용하고, 온보딩 RPC 안에서만 필수를 강제한다.
BEGIN;
...
ALTER TABLE public.users
  ADD COLUMN IF NOT EXISTS devices TEXT[] NOT NULL DEFAULT '{}'::TEXT[];
...
ALTER TABLE public.users
  DROP CONSTRAINT IF EXISTS users_age_range_valid;
ALTER TABLE public.users
  ADD CONSTRAINT users_age_range_valid CHECK (
    age_range IS NULL
    OR age_range IN ('10s', '20s', '30s', '40s', '50s', '60s_plus')
  );

ALTER TABLE public.users
  DROP CONSTRAINT IF EXISTS users_devices_valid;
ALTER TABLE public.users
  ADD CONSTRAINT users_devices_valid CHECK (
