...
const migration = readFileSync(
  new URL(
    '../supabase/migrations/20260914203000_add_account_state_and_onboarding_rpcs.sql',
    import.meta.url
  ),
  'utf8'
);
...
test('onboarding completion records terms and profile state atomically for auth.uid', () => {
  for (const sql of [migration, schema]) {
    ...
    assert.match(functionSql, /PERFORM public\.record_my_current_term_consents/i);
    ...
    assert.ok(
      functionSql.indexOf('record_my_current_term_consents')
      < functionSql.indexOf('UPDATE public.users')
    );
  }
});
